...
export default {
  ...
  mounted () {
    ...
    if (!(token && token != "undefined") || !(userStr && userStr != "undefined")) {
      ...
      this.isLogin = true;
      userApiApi.getUserFocus(this.teacher.id).then((response) => {
  ...
  methods: {
    focusTeacherClick () {
      ...
      if (this.forbiden) {
        ...
        if (this.isLogin) {
          if (this.isFocus) {
            userApiApi.cancleUserFocus(this.teacher.id).then((response) => {
              this.isFocus = response.data.focus;
              this.teacher.focus--;
              this.$message({
                message: "取消关闭老师成功哈~",
            ...
            userApiApi.addUserFocus(this.teacher.id).then((response) => {
              this.isFocus = response.data.focus;
              this.teacher.focus++;
              this.$message({
                message: "关注老师成功哈~",
